    ...
    <>
      ...
      >
        <div className="mx-auto px-4 flex items-center justify-between h-[76px] w-11/12 max-w-[1400px]">
          ...
          {/* Desktop nav */}
          <div className="relative hidden min-[1251px]:flex gap-[4px] items-center">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className={`relative px-3 py-2 rounded-[8px] text-[11px] font-bold tracking-wider uppercase transition-all duration-200 ${
                  pathname === link.href
